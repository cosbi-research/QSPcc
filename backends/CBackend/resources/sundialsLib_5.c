...
SUN_INIT *sun_init_d(SUN_INIT *s, int (*f)(realtype t, N_Vector y, N_Vector ydot, void *user_data), double *y, int y_len, double tinit_as_double, double tend_as_double, INTEGRATION_OPTS* opts, void *userdata)
{
   ...
   /* Create dense SUNLinearSolver object for use by CVode */
    LS = SUNLinSol_LapackDense(s->y, A);
    s->linearsolver = LS; /* save for deallocation */
    if(check_flag((void *)LS, "SUNDenseLinearSolver", 0)) return s;
    ...
    LS = SUNLinSol_SPGMR(s->y, PREC_LEFT, SUN_SPGMR);
#else
    LS = SUNLinSol_SPGMR(s->y, PREC_NONE, SUN_SPGMR);
#endif
    s->linearsolver = LS; /* save for deallocation */
    if(check_flag((void *)LS, "SUNDenseLinearSolver", 0)) return s;
    ...
    /* Create dense SUNMatrix for use in linear solves */
    A = SUNBandMatrix(y_len, SUN_LAPACK_BAND, SUN_LAPACK_BAND);
    s->matrix = A; /* save for deallocation */
    if(check_flag((void *)A, "SUNBandMatrix", 0)) return s;

    /* Create dense SUNLinearSolver object for use by CVode */
    LS = SUNLinSol_LapackBand(s->y, A);
    s->linearsolver = LS; /* save for deallocation */
    ...
    /* Create dense SUNMatrix for use in linear solves */
    A = SUNBandMatrix(y_len, SUN_BAND, SUN_BAND);
    s->matrix = A; /* save for deallocation */
    if(check_flag((void *)A, "SUNBandMatrix", 0)) return s;

    /* Create dense SUNLinearSolver object for use by CVode */
    LS = SUNLinSol_Band(s->y, A);
    s->linearsolver = LS; /* save for deallocation */
    if(check_flag((void *)LS, "SUNBandLinearSolver", 0)) return s;
    ...
    NLS = SUNNonlinSol_FixedPoint(s->y, SUN_NONLIN_FIXEDPOINT);
